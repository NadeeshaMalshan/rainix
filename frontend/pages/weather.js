...
export default function WeatherDashboard() {
  ...
  const handleSuggestionClick = (suggestion) => {
    ...
    setShowSuggestions(false);
    router.push(`/weather?city=${encodeURIComponent(suggestion.name)}`);
  };
  ...
  return (
    <div className="min-h-screen relative overflow-hidden font-sans text-white transition-all duration-1000 ease-in-out">
      ...
      <div 
        className={`font-poppins overflow-hidden w-screen h-screen relative flex flex-col justify-between select-none transition-all duration-1000 ${textColorClass}`}
      ...
      >
        ...
        <div ref={celestialRef} className={`absolute z-10 pointer-events-none animate-pop-in ${['cloudy', 'thunderstorm', 'rainy', 'snow'].includes(weatherState) ? 'hidden' : ''}`} style={{ left: `${orbit.x}%`, top: `${orbit.y}%` }}>
          {orbit.isNight ? (
            <img src="/images/moon.png" alt="Moon" className="w-56 h-56 object-contain" />
          ) : (
            <img src="/images/sun.png" alt="Sun" className="w-56 h-56 object-contain" />
          )}
        </div>
        ...
        {cloudConfig.count > 0 && (
          <div className="cloud-container absolute inset-0 pointer-events-none z-20">
            {Array.from({ length: cloudConfig.count }).map((_, i) => {
              const sizes = [ { w: 'w-[32rem]', h: 'h-[16rem]', top: '5%' }, { w: 'w-[45rem]', h: 'h-[20rem]', top: '15%' }, { w: 'w-[36rem]', h: 'h-[18rem]', top: '25%' }, { w: 'w-[50rem]', h: 'h-[24rem]', top: '35%' }, { w: 'w-[34rem]', h: 'h-[16rem]', top: '12%' }, { w: 'w-[40rem]', h: 'h-[18rem]', top: '45%' }, { w: 'w-[38rem]', h: 'h-[17rem]', top: '8%' } ];
              const c = sizes[i % sizes.length];
              // Reduced opacity across all clouds
        ...
        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-50 w-full max-w-sm text-center">
          <form onSubmit={handleSearchSubmit} className="flex w-[90%] max-w-md mx-auto items-start gap-3">
            <div className="flex-1 relative">
              <div 
                className="w-full h-[3.25rem] rounded-full bg-white/20 backdrop-blur-md border border-white/30 shadow-lg transition-all duration-300 flex items-center pr-2 pl-5" 
              >
                <input 
                  className={`flex-1 min-w-0 bg-transparent border-none text-left focus:ring-0 ${textColorClass} font-medium text-lg placeholder:${textColorClass} placeholder:opacity-60 outline-none`}
                ...
                />
                <button type="submit" className={`p-1.5 rounded-full hover:bg-white/30 transition-colors ${textColorClass} flex items-center justify-center`}>
              ...
              {showSuggestions && suggestions.length > 0 && (
                <div 
                  className="absolute top-[3.75rem] left-0 w-full rounded-2xl bg-black/20 backdrop-blur-xl border border-white/20 shadow-2xl transition-all duration-300 z-50 flex flex-col overflow-hidden"
                >
                  {suggestions.map((s, idx) => (
                    <div 
                      key={idx} 
                      className={`px-5 py-3 cursor-pointer hover:bg-white/20 transition-colors text-left ${textColorClass} border-b border-white/10 last:border-b-0 flex flex-col`}
                      onMouseDown={(e) => e.preventDefault()} // prevent input blur before click
                      onClick={() => handleSuggestionClick(s)}
                    >
                      <span className="font-semibold text-lg">{s.name}</span>
                      <span className="text-sm opacity-75">{s.admin1 ? `${s.admin1}, ` : ''}{s.country}</span>
                    </div>
                  ))}
                </div>
              )}
            ...
            <button 
              type="button" 
              className={`h-[3.25rem] w-[3.25rem] rounded-full flex-shrink-0 bg-white/20 backdrop-blur-md border border-white/30 hover:bg-white/30 transition-all duration-300 flex items-center justify-center ${textColorClass} shadow-lg`} 
        ...
        {!isLoading && !errorMsg && cityData && (
          <div className="flex-1 flex flex-col justify-center items-center w-full h-full relative z-40 main-weather-content animate-fade-in-up">
            ...
            <div className="flex flex-col items-center">
              <h1 className={`text-4xl md:text-4xl font-normal tracking-tight ${textColorClass} mb-1 drop-shadow-md`}>
                {formattedCity}
              </h1>
              <h2 className={`text-lg md:text-xl font-normal opacity-90 ${textColorClass} mb-8 drop-shadow-sm`}>
                {formattedCountry}
              </h2>
              ...
              <div className="flex flex-row items-center mt-4 gap-6">
                <span className={`text-2xl font-normal ${textColorClass} mb-2 `}>{weatherCondition.label}</span>
                
                <div className={`flex items-center gap-4 text-base font-normal ${textColorClass}`}>
                  <span className="flex items-center gap-1"><span className="material-symbols-outlined text-xl">arrow_upward</span>{Math.round(weather?.high || currentTemp + 6)}</span>
                  <span className="flex items-center gap-1"><span className="material-symbols-outlined text-xl">arrow_downward</span>{Math.round(weather?.low || currentTemp - 2)}</span>
                  <span className="flex items-center gap-1"><span className="material-symbols-outlined text-xl">thermostat</span>{Math.round(weather?.feelsLike || currentTemp)}</span>
                  <span className="flex items-center gap-1"><span className="material-symbols-outlined text-xl">eco</span>{aqiText}</span>
                  <span className="flex items-center gap-1"><span className="material-symbols-outlined text-xl">water_drop</span>{weather?.precipitationProbability || 0}%</span>
                </div>
              </div>

              <div>
                {hasActiveWarning && (
                  <div className="absolute  -translate-x-1/2 w-full flex items-center justify-center gap-2  text-sm md:text-base font-normal select-none pointer-events-auto">
              ...
              {/* AI Assistant Input */}
              <div className="w-full mt-6 px-4 animate-fade-in-up" style={{ animationDelay: '0.4s' }}>
                <PromptInputBasic />
              </div>
            </div>
          </div>
        ...
        <div className="absolute bottom-0 left-0 w-full z-30 pointer-events-none">
          <img src="/images/ground.png" alt="Earth Ground" className={`w-full h-auto object-cover max-h-[35vh] transition-all duration-1000 ${groundFilterClass}`} />
