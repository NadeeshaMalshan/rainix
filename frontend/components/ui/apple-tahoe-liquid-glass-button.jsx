...
const glassButtonVariants = cva(
  "relative isolate inline-flex items-center justify-center gap-2 rounded-full cursor-pointer transition-all duration-500 ease-[cubic-bezier(0.19,1,0.22,1)] active:scale-[0.92] tracking-tight disabled:opacity-50 disabled:cursor-not-allowed focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-foreground/50",
  {
    variants: {
      size: {
    ...
    return (
      <>
        ...
        <style>{`
          ...
          .btn-liquid-lens {
            background-color: ${glassColor || "rgba(255, 255, 255, 0.1)"};
...
const GlassContainer = React.forwardRef(
  ({ className, children, contentClassName, glassColor, ...props }, ref) => {
    ...
    return (
      <>
        <svg className="absolute w-0 h-0 overflow-hidden pointer-events-none" aria-hidden="true">
          <filter id={`liquid-glass-container-${filterId}`} primitiveUnits="objectBoundingBox">
            <feImage 
              result="map" 
              width="100%" 
              height="100%" 
        ...
        <div
          className={cn("relative isolate rounded-full btn-liquid", className)}
          ref={ref}
          {...props}
        >
          <span 
            className="btn-liquid-lens absolute inset-0 -z-10 rounded-[inherit] pointer-events-none" 
            style={{ 
              backgroundColor: glassColor || "rgba(255, 255, 255, 0.1)",
              backdropFilter: `blur(8px) url(#liquid-glass-container-${filterId}) saturate(150%)`,
...
export { GlassButton, GlassContainer, glassButtonVariants };
